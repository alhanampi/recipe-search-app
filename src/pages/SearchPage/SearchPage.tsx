...
const SearchPage = () => {
  ...
  useEffect(() => {
    ...
    const cached = localStorage.getItem(sessionKey('search', decoded, language));
    if (cached) {
      const {
        recipes: saved,
        totalResults: total,
    ...
    setLoading(true);
    searchRecipes(decoded, 0, language).then(
      ({ recipes: data, totalResults: total }) => {
        setRecipes(data);
        setTotalResults(total);
        localStorage.setItem(
          sessionKey('search', decoded, language),
          JSON.stringify({
            recipes: data,
            totalResults: total,
            timestamp: Date.now(),
  ...
  const handleShowMore = () => {
    setLoadingMore(true);
    searchRecipes(decoded, recipes.length, language).then(
      ({ recipes: data, totalResults: total }) => {
